...
def load_data(file_path):
    ...
    for item in data:
        ...
        output = item['output']
        inputs.append(f"任务：抽取仇恨言论四元组（评论对象 | 论点 | 目标群体 | hate/non-hate），文本如下：{content}")
...
def main():
    ...
    for fold in range(fold_num):
        ...
        best_save_path = f'./best_model_3/fold{fold}'
        if best_model_dir is not None:
            shutil.copytree(best_model_dir, best_save_path, dirs_exist_ok=True)
            print(f"Fold {fold} 最优模型已保存到: {best_save_path}")
        ...
        fold_results.append({
            "fold": fold,
            "f1": f1,
            "model_path": best_save_path
        })

    # 选出F1最高的模型
    best_fold = max(fold_results, key=lambda x: x["f1"])
    final_save_path = "./best_model_3"
    shutil.copytree(best_fold["model_path"], final_save_path, dirs_exist_ok=True)
    print(f"\n✅ 所有折中最优模型来自 Fold {best_fold['fold']}，F1 = {best_fold['f1']:.4f}")
    print(f"✅ 已将最优模型复制到: {final_save_path}")
